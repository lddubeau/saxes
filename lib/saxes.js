...
// Doing this, given we have a codepoint in the first place, is much faster than
// using a regexp.
function isChar(c) {
  return (c === TAB ||
          c === NL ||
 ...
 * track namespaces.
 *
 * @property {Object.<string, Attribute> | Object.<string, string>} attributes A
 * map of attribute name to attributes. If namespaces are tracked, the values in
 * the map are {@link Attribute Attribute} objects. Otherwise, they are strings.
 *
 ...
 * would have ``"a"`` for ``prefix``. Undefined if we do not track namespaces.
 *
 * @property {string} local The attribute's local name. For instance ``a:b="c"``
 * would have ``"b"`` for ``local``. Undefined if we do not track namespaces.
 *
 * @property {string} uri The namespace URI of this attribute. Undefined if we
 * do not track namespaces.
 *
...
class SaxesParser {
  /**
   * @param {Options} opt The parser options.
   */
  constructor(opt) {
    this._init(opt);
  }
