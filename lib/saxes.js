...
/^[A-Za-z0-9][A-Za-z0-9._-]*$/.");
  ...
  sOpenTag() {
    ...
    const c = this.captureName("tagName");
    if (!c) {
      return;
    }
  ...
  sAttribName() {
    ...
    const c = this.captureName("attribName");
    if (c === EQUAL) {
      ...
      this.attribList.push({ name: this.attribName, value: this.attribName });
  ...
  sAttribValueQuoted() {
    ...
    if (c === AMP) {
      ...
      this.attribList.push({ name: this.attribName, value: this.attribValue });
      this.attribName = this.attribValue = "";
      this.q = null;
  ...
  /** @private */
  sCloseTag() {
    const c = this.captureTo(CLOSE_TAG_TERMINATOR, "tagName");
    if (c === GREATER) {
      this.closeTag();
    }
    else if (isS(c)) {
