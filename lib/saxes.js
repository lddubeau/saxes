...
const S_BEGIN = 0; // leading byte order mark or whitespace
const S_BEGIN_WHITESPACE = 1; // leading whitespace
const S_TEXT = 2; // general stuff
const S_ENTITY = 3; // &amp and such.
const S_OPEN_WAKA = 4; // <
const S_OPEN_WAKA_BANG = 5; // <!...
const S_DOCTYPE = 6; // <!DOCTYPE
const S_DOCTYPE_QUOTED = 7; // <!DOCTYPE "//blah
const S_DOCTYPE_DTD = 8; // <!DOCTYPE "//blah" [ ...
const S_DOCTYPE_DTD_QUOTED = 9; // <!DOCTYPE "//blah" [ "foo
const S_COMMENT = 10; // <!--
const S_COMMENT_ENDING = 11; // <!-- blah -
const S_COMMENT_ENDED = 12; // <!-- blah --
const S_CDATA = 13; // <![CDATA[ something
const S_CDATA_ENDING = 14; // ]
const S_CDATA_ENDING_2 = 15; // ]]
const S_PI = 16; // <?hi
const S_PI_BODY = 17; // <?hi there
const S_PI_ENDING = 18; // <?hi "there" ?
const S_OPEN_TAG = 19; // <strong
const S_OPEN_TAG_SLASH = 20; // <strong /
const S_ATTRIB = 21; // <a
const S_ATTRIB_NAME = 22; // <a foo
const S_ATTRIB_NAME_SAW_WHITE = 23; // <a foo _
const S_ATTRIB_VALUE = 24; // <a foo=
const S_ATTRIB_VALUE_QUOTED = 25; // <a foo="bar
const S_ATTRIB_VALUE_CLOSED = 26; // <a foo="bar"
const S_ATTRIB_VALUE_UNQUOTED = 27; // <a foo=bar
const S_CLOSE_TAG = 28; // </a
const S_CLOSE_TAG_SAW_WHITE = 29; // </a   >
const S_XML_DECL_NAME_START = 30; // <?xml
const S_XML_DECL_NAME = 31; // <?xml foo
const S_XML_DECL_EQ = 32; // <?xml foo=
const S_XML_DECL_VALUE_START = 33; // <?xml foo=
const S_XML_DECL_VALUE = 34; // <?xml foo="bar"
...
class SAXParser {
  ...
  write(chunk) {
    ...
    while (i < limit) {
      let c = this.c = this.getChar(chunk, i);
      let skip = c.length;
      i += skip;
      ...
      case S_BEGIN_WHITESPACE:
        while (i < limit && isWhitespace(c)) {
          c = this.getChar(chunk, i);
          i += c.length;
      ...
      case S_TEXT:
        if (this.sawRoot && !this.closedRoot) {
          ...
          const fragment = chunk.substring(starti, i - skip);
      ...
      case S_DOCTYPE: {
        const starti = i - skip;
        while (i < limit && c !== "[" && !isQuote(c) && c !== ">") {
          c = this.getChar(chunk, i);
          skip = c.length;
          i += skip;
        }
        this.doctype += chunk.substring(starti, i - skip);
        if (c === ">") {
          this.state = S_TEXT;
      ...
      case S_PI:
        if (c === "?" || isWhitespace(c)) {
          ...
          if (!((this.piTarget.length ? NAME_CHAR : NAME_START_CHAR).test(c) &&
