...
declaration, the corresponding value was not set by the declaration. You can
listen to the `xmldecl` event ot be notified when the XML declaration is parsed.
In a well-formed document, the XML declaration may be preceded only by an optional
BOM. So by the time any event generated by the parser happens, the declaration has
been processed if present at all. Otherwise, you have a malformed document, and as
stated above, you cannot rely on the parser data!
